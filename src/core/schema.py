    ...
    def normalize_dataframe(cls, df: pl.DataFrame) -> pl.DataFrame:
        ...
        for col_name, col_type in schema.items():
            if col_name not in df.columns:
                ...
                df = df.with_columns(pl.lit(default_value).alias(col_name).cast(col_type))
...
def create_schema_dataframe(metadata: Dict[str, Any], records: List[Dict[str, Any]]) -> pl.DataFrame:
    """
    ...
    for record in records:
        ...
        normalized_record.update({
            "serial_number": metadata.get("serial_number", default_values["serial_number"]),
            "time_created": metadata.get("time_created", default_values["time_created"]),
            "total_distance": metadata.get("total_distance", default_values["total_distance"]),
            "total_timer_time": metadata.get("total_timer_time", default_values["total_timer_time"]),
            "total_calories": metadata.get("total_calories", default_values["total_calories"]),
            "avg_heart_rate": metadata.get("avg_heart_rate", default_values["avg_heart_rate"]),
            "max_heart_rate": metadata.get("max_heart_rate", default_values["max_heart_rate"]),
            "record_count": metadata.get("record_count", default_values["record_count"])
        })
