...
def main():
    ...
    parser.add_argument(
        "--user-id", type=str, default=DEFAULT_USER_ID, help="COROS user ID"
    )
    parser.add_argument(
        "--label-ids",
        type=str,
    ...
    if args.json_output:
        ...
        print(f"\n{'=' * 50}")
        print("  COROS Activity Downloader")
    ...
    if args.validate_only:
        if args.json_output:
            result = {
                ...
                "files": [{"labelId": lid, **info} for lid, info in existing.items()],
            }
            ...
            "skipped": [
                {
                    "labelId": s["labelId"],
                    "name": s["name"],
                    "existingSizeKB": s.get("existing_size_kb"),
                }
                for s in skipped
            ],
            "errors": failed,
