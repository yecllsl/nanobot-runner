...
| 🔮 **ML增强预测** | VDOT/比赛/伤病预测，三层降级架构（sklearn 特征重要性降级） | v0.20 |
| 🧬 **数字孪生** | 5维度状态向量、What-If推演、计划对比 | v0.21 |
| 🧬 **自适应进化** | 决策追踪→个性化学习→自适应进化闭环 | v0.23-v0.25 |
| 🔧 **底座升级** | nanobot-ai 0.2.0、GoalState追踪、推理可见化、Model Presets | v0.26 |
| 🌐 **WebUI 基础** | 浏览器端 AI 对话、WebSocket 通道、品牌自定义 | v0.27 |
| 📊 **WebUI 数据可视化** | Dashboard/VDOT/训练负荷/活动/身体信号 6 大页面 | v0.28 |
...
uv run nanobotrun gateway start --webui             # 启用WebUI（AI对话8765 + 数据可视化8766）
```

### WebUI 数据可视化 (v0.28.0)

启用 `--webui` 后，除 AI 对话外，还可通过浏览器访问跑步数据可视化页面：
