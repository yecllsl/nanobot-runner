...
- 🏗️ **模块化架构**: Core层六大子模块，高内聚低耦合 (v0.16.0)
- 🎣 **Hook组合系统**: 多Hook注册、流式输出、进度追踪、错误处理 (v0.17.0)
- 🤖 **Subagent架构**: 子智能体管理，支持动态任务委派 (v0.17.0)
- ⏰ **Cron训练提醒**: 基于训练计划的智能提醒调度 (v0.17.0)
- 📊 **身体信号分析**: HRV分析、疲劳度评估、恢复状态监控、身体信号智能解读 (v0.19.0)
- 🔮 **ML增强预测**: VDOT趋势预测、比赛成绩预测、伤病风险预测，三层降级架构，数据充足时自动启用ML模型 (v0.20.0-v0.20.1)
- 🧬 **自适应进化引擎**: 决策自动记录、结果回填、用户反馈收集、执行忠实度评估 (v0.23.0)
- 🧪 **测试验证体系**: 全量单元测试基线（3937 cases）、覆盖率81%、上线门禁7项全达标 (v0.24.0)
...
```bash
# 指定版本安装
curl -fsSL https://raw.githubusercontent.com/yecllsl/nanobot-runner/main/scripts/install.sh | bash -s -- --version v0.20.0
...
│   │   ├── models/         # 模型模块（v0.16.0 拆分）
│   │   │   ├── user_profile.py
│   │   │   ├── training_plan.py
│   │   │   └── analytics.py
│   │   ├── transparency/   # AI决策透明化（v0.15.0）
│   │   ├── plan/           # 智能跑步计划（v0.10.0~v0.12.0）
│   │   └── tools/          # 工具生态（v0.13.0）
