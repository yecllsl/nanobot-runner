...
def run_e2e_tests():
    ...
    success1, output1 = run_command(
        "python tests/e2e/test_user_journey.py", "E2E用户旅程测试", timeout=600
    )

    # 再执行性能测试
    success2, output2 = run_command(
        "python tests/e2e/test_performance.py", "E2E性能测试", timeout=600
    )

    return (
        success1 and success2,
        f"用户旅程: {'通过' if success1 else '失败'}, 性能测试: {'通过' if success2 else '失败'}",
    )
