...
class TestStatsAggregation:
    ...
    def test_single_run_aggregation(
    ...
    def test_multiple_runs_aggregation(
        self, mock_storage, multiple_runs_data
    ):
        """测试多次跑步的聚合统计"""
        # 保存数据
    ...
    def test_sampling_points_not_counted_as_runs(
        self, mock_storage, sample_run_data
    ):
        """测试采样点不会被误认为跑步次数"""
        # 保存数据（3600 个采样点）
    ...
    def test_distance_not_doubly_counted(
        self, mock_storage, sample_run_data
    ):
        """测试距离不会被重复计算"""
    ...
    def test_time_not_doubly_counted(
        self, mock_storage, sample_run_data
    ):
        """测试时长不会被重复计算"""
...
class TestStatsAggregationEdgeCases:
    ...
    def test_single_sampling_point_run(
        self, mock_storage
    ):
        """测试只有一个采样点的跑步"""
        # 创建只有 1 个采样点的数据
    ...
    def test_same_start_time_different_sessions(
        self, mock_storage
    ):
