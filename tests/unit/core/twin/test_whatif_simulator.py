...
class TestEstimateWeeklyTss:
    def test_basic_estimation(self) -> None:
        ...
        tss = WhatIfSimulator.estimate_weekly_tss(plan)
        assert tss > 0
    ...
    def test_intensity_multiplier(self) -> None:
        ...
        tss_base = WhatIfSimulator.estimate_weekly_tss(plan_base)
        tss_high = WhatIfSimulator.estimate_weekly_tss(plan_high)
...
class TestSimulateWeek:
    def test_returns_runner_state_vector(self) -> None:
        ...
        result = WhatIfSimulator.simulate_week(state, plan, "parametric")
...
class TestSimulate:
    def test_snapshot_count(self) -> None:
        ...
        result = WhatIfSimulator.simulate(state, plan, "parametric")
    ...
    def test_confidence_decay_l1(self) -> None:
        state = _make_state()
        plan = _make_plan(weeks=4)
        result = WhatIfSimulator.simulate(state, plan, "ml_enhanced")
    ...
    def test_confidence_decay_l3(self) -> None:
        state = _make_state()
        plan = _make_plan(weeks=4)
        result = WhatIfSimulator.simulate(state, plan, "basic")
        confidences = [s.confidence for s in result.snapshots]
        assert confidences[0] > confidences[-1]
    ...
    def test_l3_decays_faster_than_l1(self) -> None:
        ...
        result_l1 = WhatIfSimulator.simulate(state, plan, "ml_enhanced")
        result_l3 = WhatIfSimulator.simulate(state, plan, "basic")
