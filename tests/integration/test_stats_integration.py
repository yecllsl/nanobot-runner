...
class TestStatsCommandIntegration:
    ...
    def test_stats_command_with_multiple_runs(self, sample_parquet_data):
    ...
    def test_stats_command_year_filter(self, sample_parquet_data):
        """测试 stats 命令的年份过滤功能"""
        from src.core.analytics import AnalyticsEngine
    ...
    def test_stats_command_date_range_filter(self, sample_parquet_data):
...
class TestStatsAggregationRealData:
    ...
    def test_multiple_realistic_runs_aggregation(self, tmp_path, monkeypatch):
        """测试多次真实跑步的聚合统计"""
        storage_dir = tmp_path / "data"
        storage_dir.mkdir()
        storage = StorageManager(storage_dir)
