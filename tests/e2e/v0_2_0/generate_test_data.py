...
def generate_basic_test_data(record_count: int = 1000) -> pl.DataFrame:
    ...
    # 验证数据完整性
    required_columns = [
        'activity_id', 'timestamp', 'source_file', 'filename',
        'total_distance', 'total_timer_time', 'avg_heart_rate'
    ]
...
def validate_test_data() -> bool:
    ...
    for file_path in parquet_files:
        try:
            ...
            required_columns = [
                'activity_id', 'timestamp', 'total_distance', 
...
def main():
    ...
    parser.add_argument('--action', choices=['generate', 'validate', 'clean'], 
                       default='generate', help='执行动作')
