...
class TestAgentE2EMain:
    ...
    def test_daily_training_query_flow(self):
        ...
        assert "最近" in recent_result or "记录" in recent_result, "最近记录查询失败"
        ...
        assert "表格" in formatted_result or "面板" in formatted_result, "响应格式化失败"
        print("✅ 响应格式化正确")
    ...
    def test_performance_benchmark(self):
        ...
        for scenario_name, query_type, time_limit in query_scenarios:
            ...
            assert avg_query_time < time_limit, f"{scenario_name}平均时间{avg_query_time:.2f}秒超过{time_limit}秒限制"
