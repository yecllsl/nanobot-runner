...
| 模块 | 覆盖功能 | 优先级 |
|------|---------|--------|
| 数据导入 | 单文件/批量导入、去重、异常处理 | P0 |
| 数据查询 | 统计信息、年份/日期范围过滤 | P0 |
| 数据分析 | VDOT趋势、训练负荷、心率漂移 | P0 |
| 训练计划 | 反馈记录、统计查看、训练建议、目标评估 | P0 |
| 报告生成 | 周报、月报、用户画像 | P0 |
| 系统管理 | 版本、配置验证、初始化迁移 | P1 |
| Agent交互 | 自然语言对话、数据查询 | P1（手动） |
...
训练计划功能需要先创建计划数据才能测试。在执行UAT-012~UAT-016前，先创建测试计划：

```powershell
# 方式1：使用evaluate命令创建目标评估（会自动创建关联计划）
uv run nanobotrun plan evaluate marathon 10800 --vdot 45 --weeks 12

# 方式2：使用long-term命令创建长期训练规划
...
**说明**: 执行上述命令后，系统会自动生成计划ID。后续测试使用该计划ID。
...
| **测试目的** | 验证自然语言数据查询 |
| **前置条件** | 已进入Agent对话模式 |
| **测试输入** | 1. "我上周跑了多少次？" 2. "我的VDOT趋势如何？" |
| **预期结果** | Agent理解查询意图，返回准确数据 |
| **验收方式** | 人工观察 |
| **优先级** | P1 |
