...
| Phase D | v0.28.0 | 📋 规划中 | VDOT/负荷/身体信号图表，图表与CLI数据一致 |
...
| 风险 | 等级 | 影响 | 缓解措施 | 适用版本 |
|------|------|------|----------|----------|
| 原生 WebUI 不支持跑步数据可视化 | 中 | v0.28.0 需扩展前端组件，工作量不确定 | v0.27.0 已验证基础链路，v0.28.0 复用 Recharts/ECharts | v0.28.0 |
...
| 后端 API 与前端组件耦合 | 中 | v0.28.0 新增API 端点可能与上游变更冲突 | API 端点独立维护；明确契约；前后端通过版本化接口解耦 | v0.28.0+ |
