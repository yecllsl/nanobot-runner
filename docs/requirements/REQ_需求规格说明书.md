...
> **当前基线**: v0.33.0
> **覆盖版本**: v1.0.0
> **对齐产品规划**: v19.0 (2026-07-24)
> **对齐架构设计**: v26.0.0 (2026-07-15)
...
| 飞书Gateway增强 | 盈亏分析+结算、飞书Gateway增强 | v0.30 |
| 系统健壮性提升 | 代码质量优化、系统健壮性提升 | v0.31 |
| nanobot-ai 0.2.2升级 | 移除monkey-patch、SDK编程式调用、运行时事件总线、自定义Provider | v0.32 |
...
**当前状态**：v0.26.0-v0.32.0 全部完成，Phase D 交互升级阶段已结束。
...
### 5.7 v0.34.0：WebUI 数据导入

**背景**：当前 WebUI（8766）仅支持数据查看，导入新数据必须切回 CLI 执行 `uv run nanobotrun data import`。v0.34.0 在 WebUI 提供等价导入能力，实现"查看+导入"一体化，降低上下文切换成本。

**核心约束**：功能与 CLI `data import` 完全等价，复用同一 `ImportService`，不引入并行导入逻辑。浏览器场景无法上传"目录路径"，以"多文件选择"等价替代目录导入。

#### 5.7.1 功能需求

| 编号 | 需求 | 优先级 | 描述 |
|------|------|--------|------|
| FR-01 | 多文件选择上传 | P0 | 用户点击上传按钮，可选择 1 个或多个 `.fit` 文件；选中后自动触发导入 |
| FR-02 | 强制重新导入 | P0 | 页面提供「强制重新导入（跳过去重）」复选框，勾选后本次上传所有文件以 `force=true` 导入，等价于 CLI `--force` |
| FR-03 | 导入结果反馈 | P0 | 同步批量返回每个文件的导入状态（`added`/`skipped`/`error`）及消息；完成后显示汇总：成功 N、跳过 M、错误 K |
| FR-04 | 文件类型与大小校验 | P1 | 仅接受 `.fit` 文件；单文件 ≤ 50MB；单次上传 ≤ 50 个文件；超限拒绝并提示 |
...
|------|------|------|
| NFR-D-24 | 单文件导入耗时 | < 3s |
| NFR-D-25 | 不阻塞事件循环 | 导入操作通过 `run_in_threadpool` 包装，不阻塞其他 API |
| NFR-D-26 | 临时文件清理 | 导入完成或异常后，临时文件必须删除 |
| NFR-D-27 | 安全认证 | 复用现有 `get_current_user` token 认证，与现有 API 一致 |
| NFR-D-28 | 向后兼容 | 不影响 CLI/飞书现有 `data import` 功能 |

#### 5.7.3 验收标准

1. 用户在 WebUI 导入页点击上传按钮，可选择 1 个或多个 `.fit` 文件
2. 选中后自动导入，页面显示每个文件结果（新增/跳过/错误 + 消息）
3. 支持「强制重新导入」复选框，勾选后跳过 SHA256 去重
4. 完成后显示汇总：成功 N、跳过 M、错误 K
5. 与 CLI `data import` 完全等价（复用同一 `ImportService.import_file`）
6. 非 `.fit` 文件被拒绝并提示；超 size/数量限制被拒绝并提示
7. 需 token 认证（未认证返回 401）
8. 不影响现有 CLI/飞书功能

#### 5.7.4 技术方案要点

- **后端**：新增 `src/core/webui/routes/import_data.py`，接收 `UploadFile` 列表，逐个写入临时文件后调用 `context.importer.import_file(Path(tmp), force=force)`，收集结果后清理临时文件，用 `run_in_threadpool` 包装同步阻塞
- **依赖**：新增 `python-multipart`（FastAPI `UploadFile` 必需，无法避免）
- **前端**：新增 `pages/ImportPage.tsx` + `api/import.ts` + 路由 + 侧边栏导航项；复用 `apiClient`（自动 token）与 `useApi` hook
- **复用**：`ImportService.import_file` 返回干净 dict `{"status","message","fingerprint"}`，无需改造；其内部 `rich.Console` 输出在无 TTY 时不报错
...
| NFR-D-24 | 单文件导入耗时 | < 3s | v0.34+ |
| NFR-D-25 | 不阻塞事件循环 | 导入操作通过 run_in_threadpool 包装 | v0.34+ |
| NFR-D-26 | 临时文件清理 | 导入完成或异常后必须删除临时文件 | v0.34+ |
| NFR-D-27 | 安全认证 | 复用现有 token 认证 | v0.34+ |
| NFR-D-28 | 向后兼容 | 不影响 CLI/飞书 data import | v0.34+ |
...
| v0.34.0 | v0.34.0 | 📋 规划中 | WebUI 数据导入（多文件上传+force+结果反馈）、5项NFR达标、复用ImportService |
