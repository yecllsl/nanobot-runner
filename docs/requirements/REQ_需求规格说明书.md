...
> **最后更新**: 2026-05-20
> **当前基线**: v0.23.0
> **覆盖版本**: v0.23.0 - v0.25.0
> **对齐产品规划**: v10.0 (2026-05-20)
> **对齐架构设计**: v10.1.0 (2026-05-20)
> **外部参考**: 产品演进设计 v1.0 + 多智能体架构分析 + Phase C 设计规格书 v1.0
...
| 决策追踪模块 | 未定义 | 未定义 | **`src/core/evolution/`**（v0.23新增，递增式） |
| 个性化学习模块 | 未定义 | 未定义 | **`src/core/evolution/`**（v0.24递增式添加） |
| 进化模块 | `src/core/evolution/` | 未定义 | **`src/core/evolution/`**（v0.25递增式添加） |

**裁决理由**：prediction命名与架构设计说明书v7.1.0一致；twin为v0.21新增模块；v0.23-v0.25统一为 `src/core/evolution/` 模块，功能递增式添加（v0.23决策追踪→v0.24校准+响应分析→v0.25进化控制+提示调优），与 prediction/twin 同级，进化逻辑集中管理，避免模块碎片化。
...
| 进化状态 | `evolution status` | 无 | **`evolution status`**（v0.23） |
| 进化历史 | 无 | 无 | **`evolution history [--start] [--end] [--type]`**（v0.23） |
| 进化反馈 | 无 | 无 | **`evolution feedback <decision_id> --score [--text] [--accepted]`**（v0.23） |
| 进化准确度 | 无 | 无 | **`evolution accuracy [--days 30]`**（v0.23） |
| 进化忠实度 | 无 | 无 | **`evolution fidelity [--days 30]`**（v0.23） |
| 进化校准 | 无 | 无 | **`evolution calibration [--model-type]`**（v0.24） |
| 训练响应分析 | 无 | 无 | **`evolution response [--months 6]`**（v0.24） |
| 进化触发器 | 无 | 无 | **`evolution triggers`**（v0.25） |
| 进化报告 | 无 | 无 | **`evolution report [--month]`**（v0.25） |
| 提示调优 | 无 | 无 | **`evolution tune --tone --detail --aggressive`**（v0.25） |

**裁决理由**：CLI命令以现有需求规格为基准，保持命令组层级一致性（predict/twin/evolution各自独立命令组）。evolution命令组覆盖v0.23-v0.25全部功能，按版本递增式添加子命令。
...
| 记录反馈 | 无 | 无 | **record_feedback**（v0.23新增） |
| 检查计划执行 | 无 | 无 | **check_plan_execution**（v0.23新增） |
| 检查预测准确度 | 无 | 无 | **check_prediction_accuracy**（v0.23新增） |
| 查询决策历史 | 无 | 无 | **get_decision_history**（v0.23新增） |
| 分析训练响应 | 无 | 无 | **analyze_training_response**（v0.24新增） |
| 查看校准状态 | 无 | 无 | **get_calibration_status**（v0.24新增） |
| 检查进化触发 | 无 | 无 | **check_evolution_triggers**（v0.25新增） |
| 获取进化报告 | 无 | 无 | **get_evolution_report**（v0.25新增） |
| 调整提示参数 | 无 | 无 | **adjust_prompt_params**（v0.25新增） |

**裁决理由**：Agent工具命名遵循snake_case风格，与现有工具命名规范一致。v0.23-v0.25进化模块工具统一归属evolution模块，按版本递增式注册。
...
**版本主题**: 决策-结果追踪 —— 记录AI决策全链路，建立"决策→执行→结果"闭环
**核心目标**: 让系统具备自我评估能力，为后续个性化学习与校准提供数据基础
**前置依赖**: v0.20预测引擎、v0.21孪生引擎（可选）
**⚠️ 不依赖v0.22**: 即使v0.22跳过某些功能，v0.23仍可独立交付
**模块归属**: `src/core/evolution/`（v0.23递增式添加决策追踪子模块）

#### 4.1.2 P0需求：决策追踪核心

##### REQ-0.23-01：决策日志

**需求描述**: 记录每次AI决策的完整上下文，包括跑者状态摘要、工具调用链、预测快照。采用 DecisionLog + OutcomeRecord 分离模型，职责清晰、查询高效、与存储分片自然对齐。

**功能要点**:

| 字段 | 说明 |
|------|------|
| decision_id | UUID唯一标识 |
| timestamp | 决策发生时间 |
| runner_state | RunnerStateVector摘要快照（dict） |
| decision_type | 复用现有 DecisionType 枚举（TRAINING_ADVICE/PLAN_ADJUSTMENT/RECOVERY_SUGGESTION/WEATHER_ADVICE/DATA_QUERY/GENERAL） |
| tool_call_chain | 本次决策调用的所有工具及参数 [{name, args, result_summary}] |
| prediction_snapshot | 决策时做出的预测快照（如有） |
| recommendation_text | 推荐文本摘要 |
| execution_status | 执行状态（pending/executed/skipped/modified） |
| recommendation_accepted | 用户是否采纳推荐 |
| session_key | 会话标识 |

**验收标准**:

- [ ] AC-01: DecisionLog为frozen dataclass，包含上述全部字段；OutcomeRecord为独立frozen dataclass，通过decision_id关联
- [ ] AC-02: 通过DecisionLogHook无侵入接入，继承ObservabilityHook，所有方法先调super()再扩展，不修改核心Agent逻辑
- [ ] AC-03: 决策日志按月分片存储为Parquet格式：`~/.nanobot-runner/decisions/2026-05/`
- [ ] AC-04: DecisionLogHook实现before_iteration/before_execute_tools/finalize_content三个钩子
- [ ] AC-05: DecisionType复用现有transparency模块的DecisionType枚举，不新建枚举
- [ ] AC-06: runner_state仅存储RunnerStateVector摘要（dict），不存储完整状态向量对象，控制存储膨胀
...
**需求描述**: 对比计划vs实际、预测vs实际，建立结果追踪闭环。反馈收集复用AskUserConfirmManager。

**功能要点**:

| 功能 | 说明 |
|------|------|
| check_plan_execution() | 对比计划训练vs实际训练，计算执行忠实度（fidelity = 1 - weighted_avg(体积偏差, 强度偏差, 时间偏差)） |
| check_prediction_accuracy() | 对比预测VDOT vs 实际VDOT，对比预测伤病风险vs实际伤病事件 |
| record_feedback() | 记录用户反馈（评分1-5星、文本、是否采纳） |
| generate_feedback_prompt() | 复用AskUserConfirmManager生成反馈收集提示，扩展ConfirmScenario.DECISION_FEEDBACK |

**验收标准**:

- [ ] AC-01: check_plan_execution()输出OutcomeRecord，含执行忠实度(0-1)、偏差详情
- [ ] AC-02: check_prediction_accuracy()输出OutcomeRecord，含MAE、偏差方向
- [ ] AC-03: 结果回填不阻塞主流程，异步执行
- [ ] AC-04: 结果记录按月分片存储为Parquet格式：`~/.nanobot-runner/outcomes/2026-05/`
- [ ] AC-05: generate_feedback_prompt()复用AskUserConfirmManager，扩展ConfirmScenario枚举新增DECISION_FEEDBACK场景
- [ ] AC-06: record_feedback()支持评分(1-5)、文本反馈、是否采纳三个维度
- [ ] AC-07: 反馈收集率>30%（通过AskUserConfirmManager轻量反馈机制实现）
...
| 反馈收集 | 复用AskUserConfirmManager，反馈收集率>30% |
...
**前置依赖**: v0.23决策追踪系统
**模块归属**: `src/core/evolution/`（v0.24递增式添加校准引擎+响应分析器）
...
- [ ] AC-02: 基于v0.23决策日志中的训练-结果配对数据（筛选execution_fidelity ≥ 0.7）
- [ ] AC-03: 输出个人训练响应画像（如"间歇训练响应性强"）
- [ ] AC-04: 分析结果可被LLM引用，用于个性化训练计划生成
- [ ] AC-05: 最低样本数门槛：每组训练类型≥5条数据，不足时返回"数据不足"

---

#### 4.2.3 P1需求：预测校准与模型进化

##### REQ-0.24-02：预测校准层

**需求描述**: 基于决策日志和结果记录，校准预测模型的系统性偏差。在预测输出上加 bias + scale 线性修正。

**验收标准**:

- [ ] AC-01: 校准器检测预测的系统性偏差（持续高估/低估），输出偏差方向和幅度
- [ ] AC-02: 校准结果应用于后续预测，自动修正预测值（corrected = raw × scale + bias）
- [ ] AC-03: 校准触发条件：累计≥10条预测-实际配对数据
- [ ] AC-04: 校准过程输出校准报告，含修正前后对比
- [ ] AC-05: 校准参数使用指数移动平均更新（α=0.7），保证稳定性
- [ ] AC-06: 校准幅度上限±10%，防止过度修正
...
| 工具名 | 功能 | 输入 | 输出 |
|--------|------|------|------|
| analyze_training_response | 分析训练响应性 | months?(默认6) | TrainingResponseReport |
| get_calibration_status | 查看校准配置与效果 | model_type? | CalibrationProfile |
...
**模块归属**: `src/core/evolution/`（v0.25递增式添加进化控制器+提示调优器）
...
- [ ] AC-01: VDOT预测误差连续3次>5%时，自动触发对应模型重训练
...
**需求描述**: 基于用户反馈和决策效果，自动优化LLM提示词（4维参数空间：语气/详细度/激进程度/数据驱动权重）

**验收标准**:

- [ ] AC-01: 个性化语气：根据用户偏好调整建议风格（严厉/温和/数据驱动），tone_intensity 0.0-1.0
- [ ] AC-02: 信息密度：根据用户反馈调整输出详细程度，detail_level_score 0.0-1.0
- [ ] AC-03: 推荐策略：根据采纳率调整推荐激进程度，recommendation_aggressiveness 0.0-1.0
- [ ] AC-04: 优化策略存储为配置文件，可回滚（reset_to_default()）
...
- [ ] AC-01: CLI命令 `evolution status` 输出进化引擎状态
- [ ] AC-02: 展示：决策记录数、预测准确率趋势、决策接受率、模型版本、个性化程度、上次进化时间
- [ ] AC-03: CLI命令 `evolution triggers` 手动触发进化检查
...
| 工具名 | 功能 | 输入 | 输出 |
|--------|------|------|------|
| check_evolution_triggers | 检查进化触发条件 | 无 | list[EvolutionAction] |
| get_evolution_report | 获取月度进化报告 | month? | EvolutionReport |
| adjust_prompt_params | 手动调整提示参数 | tone?, detail?, aggressive? | PromptTuningParams |
...
| NFR-12c | 进化模块无侵入 | evolution模块通过DecisionLogHook(ObservabilityHook子类)接入，所有方法先调super()再扩展 | v0.23+ |
...
| 数据项 | 类型 | 说明 | 来源 |
|--------|------|------|------|
| decision_id | str | UUID唯一标识 | DecisionLogHook自动生成 |
| timestamp | datetime | 决策发生时间 | DecisionLogHook自动采集 |
| runner_state | dict | RunnerStateVector摘要快照 | twin_engine.get_current_snapshot() |
| decision_type | DecisionType | 决策类型枚举 | 复用transparency模块DecisionType |
| tool_call_chain | list[dict] | 工具调用链 [{name, args, result_summary}] | DecisionLogHook提取 |
| prediction_snapshot | dict | None | 预测快照 | prediction_engine查询 |
| recommendation_text | str | None | 推荐文本摘要 | Agent输出解析 |
| execution_status | str | 执行状态(pending/executed/skipped/modified) | OutcomeCollector更新 |
| recommendation_accepted | bool | None | 用户是否采纳推荐 | 用户反馈 |
| session_key | str | 会话标识 | Agent上下文 |

**OutcomeRecord 字段**:

| 数据项 | 类型 | 说明 | 来源 |
|--------|------|------|------|
| outcome_id | str | UUID唯一标识 | OutcomeCollector自动生成 |
| decision_id | str | FK关联DecisionLog | 关联决策记录 |
| outcome_timestamp | datetime | 结果时间 | 回填时自动采集 |
| actual_vdot | float | None | 实际VDOT | 训练记录 |
| actual_injury | bool | 是否发生伤病 | 伤病标签/用户反馈 |
| execution_fidelity | float | None | 执行忠实度(0-1) | check_plan_execution()计算 |
| user_feedback_score | int | None | 用户评分(1-5星) | AskUserConfirmManager收集 |
| user_feedback_text | str | None | 文本反馈 | AskUserConfirmManager收集 |
| prediction_error | float | None | 预测误差% | check_prediction_accuracy()计算 |
| session_id | str | None | 关联训练Session | 训练记录关联 |

### 7.4 新增存储需求

| 数据类型 | 存储位置 | 估算大小 | 版本 |
|---------|---------|---------|------|
| ML模型文件 | ~/.nanobot-runner/models/ | 5-50MB/模型 | v0.20 |
| 预测历史记录 | ~/.nanobot-runner/predictions/ | ~1MB/年 | v0.20 |
| 特征缓存 | ~/.nanobot-runner/cache/ | ~10MB | v0.20 |
| 伤病标签 | ~/.nanobot-runner/injury_labels/ | ~1MB/年 | v0.20 |
| 决策日志 | ~/.nanobot-runner/decisions/ | ~5MB/年 | v0.23 |
| 结果记录 | ~/.nanobot-runner/outcomes/ | ~2MB/年 | v0.23 |
| 状态向量缓存 | ~/.nanobot-runner/twin/state_vector.json | ~5KB | v0.21 |
| 推演结果缓存 | ~/.nanobot-runner/twin/simulations/ | ~50KB/次 | v0.21 |
| 校准配置 | ~/.nanobot-runner/calibrations/ | ~1KB/模型 | v0.24 |
| 提示调优参数 | ~/.nanobot-runner/tuning/ | ~1KB | v0.25 |
...
graph TD
    ...
    REQ02302 --> REQ02402[REQ-0.24-02<br/>预测校准层]
    REQ02402 --> REQ02403[REQ-0.24-03<br/>个人化模型进化]
    REQ02401 --> REQ02404[REQ-0.24-04<br/>最佳训练窗口]
...
| REQ-0.23-01 | 决策日志（DecisionLog+OutcomeRecord分离模型/DecisionLogHook/Parquet分片） | P0 | REQ-0.20-01, REQ-0.21-01 |
| REQ-0.23-02 | 结果回填机制（忠实度/准确性/AskUserConfirmManager反馈收集） | P0 | REQ-0.23-01 |
| REQ-0.24-01 | 训练响应性分析（效果排名/响应画像/最低样本门槛） | P0 | REQ-0.23-02 |
| REQ-0.24-02 | 预测校准层（偏差检测/bias+scale修正/EMA更新/幅度上限±10%） | P1 | REQ-0.23-02 |
| REQ-0.24-03 | 个人化模型进化（参数校准/进化报告） | P1 | REQ-0.24-02 |
| REQ-0.24-04 | 最佳训练窗口预测 | P2 | REQ-0.24-01 |
| REQ-0.25-01 | 自动化进化触发器（条件检测/异步触发） | P0 | REQ-0.24-01, REQ-0.24-02 |
| REQ-0.25-02 | LLM提示策略优化（4维参数空间/语气/密度/策略） | P1 | REQ-0.25-01 |
...
| v0.23 | 决策-结果追踪 | 2 | 0 | 0 | v0.20+v0.21 |
| v0.24 | 个性化学习 | 1 | 2 | 1 | v0.23 |
| v0.25 | 自适应进化引擎 | 1 | 1 | 1 | v0.23+v0.24 |

**需求总数**: P0=12, P1=6(含1项延后), P2=3, **总计=23**（v0.21交付P0=2项）

> **v9.0变更说明**：REQ-0.23-03预测校准层从v0.23移至v0.24（REQ-0.24-02），v0.24新增REQ-0.24-03个人化模型进化（原REQ-0.24-02），原REQ-0.24-03最佳训练窗口调整为REQ-0.24-04。
...
| 风险 | 等级 | 影响 | 缓解措施 | 覆盖版本 |
|------|------|------|----------|---------|
| 决策日志数据膨胀 | 高 | 长期运行后日志过大 | Parquet按月分片；>12月自动归档；runner_state仅存摘要(dict) | v0.23 |
| 用户反馈稀疏 | 高 | 缺乏足够反馈驱动进化 | 复用AskUserConfirmManager轻量反馈；隐式反馈自动记录 | v0.23-v0.25 |
| 校准偏差方向错误 | 中 | 校准后预测反而变差 | 最少10条样本才触发；EMA(α=0.7)保证稳定；校准幅度上限±10% | v0.24 |
| 进化方向偏差 | 中 | 系统学习到的偏好与真实目标偏离 | 保留人工覆盖机制；月度进化报告供review；调整幅度小(0.05-0.1)；reset_to_default()回退 | v0.25 |
| Hook扩展影响透明化 | 中 | DecisionLogHook可能干扰现有追踪 | 所有方法先调super()再扩展；twin/prediction查询失败graceful降级 | v0.23 |
| 训练响应性样本不足 | 低 | 分析结果不可靠 | 最低样本数门槛(每组≥5)；不足时返回"数据不足" | v0.24 |
...
| Hook性能 | DecisionLogHook接入对主流程延迟增加<100ms | v0.23 |
| 反馈收集 | 复用AskUserConfirmManager，反馈收集率>30% | v0.23 |
| 响应性分析 | 训练类型效果排名与用户主观感受一致率>70% | v0.24 |
| 预测校准 | 校准后预测误差降低≥10% | v0.24 |
| 模型进化 | 校准后VDOT预测MAE降低≥15% | v0.24 |
| 伤病校准 | 校准后伤病风险AUC提升≥0.05 | v0.24 |
  ...
  root((产品演进需求<br/>v0.20-v0.25))
    Phase A 数字孪生跑者
      ...
      v0.22 质量收口
        P0 UAT全面验证
        P1 缺陷收敛 需求洞察
    Phase C 自适应进化引擎 evolution/
      v0.23 决策-结果追踪
        P0 决策日志 DecisionLog+OutcomeRecord分离模型
        P0 结果回填机制 AskUserConfirmManager反馈收集
        Agent record_feedback check_plan_execution check_prediction_accuracy get_decision_history
        CLI evolution history/feedback/accuracy/fidelity/status
      v0.24 个性化学习
        P0 训练响应性分析 最低样本门槛
        P1 预测校准层 bias+scale修正 EMA更新
        P1 个人化模型进化 参数校准
        P2 最佳训练窗口
        Agent analyze_training_response get_calibration_status
        CLI evolution calibration/response
      v0.25 自适应进化引擎
        P0 进化触发器
        P1 提示策略优化 4维参数空间
        P2 进化仪表盘
        Agent check_evolution_triggers get_evolution_report adjust_prompt_params
        CLI evolution triggers/report/tune
    非功能需求
      性能 预测小于5s 推理小于100ms Hook延迟小于100ms
      安全 本地存储 不确定性声明
      可维护 无侵入接入 降级策略
      可扩展 模型可替换 特征可扩展
```
...
| DecisionLog | 决策日志，记录AI决策的完整上下文（frozen dataclass），与OutcomeRecord分离存储 |
| OutcomeRecord | 执行结果记录，通过decision_id关联DecisionLog，记录实际VDOT/伤病/忠实度/反馈 |
| DecisionLogHook | 决策日志钩子，继承ObservabilityHook，无侵入采集决策上下文 |
| EvolutionEngine | 进化引擎薄编排层，委托给DecisionTracker/OutcomeCollector/CalibrationEngine等子组件 |
| EvolutionStore | 统一存储编排，管理decisions/outcomes/calibrations/tuning的Parquet+JSON读写 |
| 执行忠实度 | 计划训练vs实际训练的吻合程度(0-1)，fidelity = 1 - weighted_avg(体积偏差, 强度偏差, 时间偏差) |
| 偏差修正层 | CalibrationEngine，在预测输出上加bias+scale线性修正，EMA(α=0.7)更新参数 |
| 参数化提示调优 | PromptTuner，4维参数空间(语气/详细度/激进程度/数据驱动权重)调整LLM输出风格 |
...
| ConfirmScenario | AskUserConfirmManager的场景枚举，v0.23新增DECISION_FEEDBACK场景 |

## 附录B：变更记录

| 版本 | 日期 | 变更内容 |
|------|------|---------|
| v9.0 | 2026-05-20 | **Phase C设计规格书对齐更新**：(1)模块命名统一为evolution/，v0.23-v0.25递增式添加；(2)REQ-0.23-03预测校准层移至v0.24(REQ-0.24-02)；(3)数据模型改为DecisionLog+OutcomeRecord分离模型；(4)复用DecisionType枚举；(5)Hook命名改为DecisionLogHook(继承ObservabilityHook)；(6)新增AskUserConfirmManager集成(扩展ConfirmScenario.DECISION_FEEDBACK)；(7)新增v0.23-v0.25 Agent工具和CLI命令；(8)更新v0.23/v0.24成功标准；(9)新增2.3节evolution命令组裁决、2.4节v0.23-v0.25 Agent工具裁决；(10)新增7.3节v0.23数据项(DecisionLog+OutcomeRecord字段)；(11)更新第8节依赖关系图和需求清单；(12)更新第9节迭代计划(v0.23 P1→0, v0.24 P1→2)；(13)更新第11节Phase C验收门禁(10项)；(14)更新第12节需求脑图；(15)新增术语(DecisionLog/OutcomeRecord/DecisionLogHook/EvolutionEngine/EvolutionStore/执行忠实度/偏差修正层/参数化提示调优/ConfirmScenario)；对齐产品规划v9.2和架构设计v9.2.0 |
