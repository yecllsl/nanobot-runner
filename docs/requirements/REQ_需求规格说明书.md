...
> **对齐产品规划**: v9.0 (2026-05-08)  
> **对齐架构设计**: v7.1.0 (2026-05-08)  
...
| 评估维度 | 评估指标 | 计算方式 |
|----------|---------|---------|
| 时间跨度 | 数据覆盖月数 | 最早记录至最近记录的时间跨度 |
| 记录数量 | 总训练记录条数 | SessionRepository查询计数 |
| 训练频率 | 平均每周训练次数 | 记录数/覆盖周数 |
| 比赛记录 | 实测比赛成绩次数 | 含race_type标记的记录数 |
| 心率完整度 | 心率数据覆盖率 | 含心率数据的记录/总记录 |
| 身体信号 | v0.19身体信号数据可用性 | BodySignalEngine数据质量检查 |

---

#### 3.1.3 P0 需求：必须交付（ML增强核心）

##### REQ-0.20-01：ML-VDOT趋势预测引擎

**需求描述**: 基于历史VDOT数据、训练负荷和身体信号，使用ML时间序列模型预测未来VDOT变化趋势，替代简单线性回归，显著提升预测准确度

**功能要点**:

| 子功能 | 说明 | ML优势 | 数据来源 |
|--------|------|--------|----------|
| 时序特征工程 | 提取训练周期、季节性、身体适应特征（≥5类） | 识别非线性趋势和周期性 | 18个月+训练数据 |
| 多因子ML模型 | 整合周跑量、强度分布、身体信号预测VDOT | 捕捉复杂交互效应 | v0.19身体信号数据 |
| 预测置信区间 | 基于分位数回归输出(p10, p50, p90)置信区间 | 更科学的区间估计 | 模型内置 |
| 特征重要性 | 展示影响VDOT的关键特征 | 可解释ML洞察 | SHAP值分析 |
| 参数化基线 | Banister IR Model冷启动（数据200-400条） | 运动科学理论支撑 | 确定性模型 |

**验收标准**:

- [ ] AC-01: 数据充足时(18个月+/400+条)，使用ML模型预测VDOT趋势，预测误差<5%（对比基础预测8%）
- [ ] AC-02: 数据不足时，自动降级为基础预测（线性回归），输出明确提示"当前数据量XX，建议积累更多数据后可解锁ML增强预测"
- [ ] AC-02b: 数据中等时(200-400条)，自动启用参数化基线(Banister IR模型)，输出提示"📊 参数化模型预测，数据积累后可升级为ML增强预测"
- [ ] AC-03: 时序特征工程提取≥5类特征（类别定义：训练周期特征、季节性特征、身体适应特征、负荷变化率特征、强度分布特征），每类≥2个具体特征
- [ ] AC-04: 预测置信区间基于分位数回归输出(p10, p50, p90)和置信度(0-1)
- [ ] AC-05: 特征重要性通过SHAP值分析，输出Top3关键特征，每项包含 name(string)、weight(float,0-1)、direction(enum: positive/negative/neutral)
...
class MLPredictionInfo:
    ...
    feature_count: int
    shap_available: bool
    quantile_models: bool  # 是否使用分位数回归
```

**前置依赖**:
- RacePredictionEngine.predict_vdot_at_race() 可复用线性回归降级方案
- RacePredictionEngine.calculate_confidence() 可复用置信度计算
- VDOTCalculator.calculate_vdot_batch() 可复用批量VDOT计算
- SessionRepository 提供历史数据查询
- BodySignalEngine(v0.19) 提供身体信号数据

---

##### REQ-0.20-02：个人化比赛成绩预测

**需求描述**: 基于个人历史比赛数据训练个人化修正模型，替代Jack Daniels固定系数，考虑个人特点（耐力型vs速度型）和赛前身体状态
...
- [ ] AC-01: 数据充足时(3次+实测比赛)，使用个人化模型预测比赛成绩，全马预测误差<8分钟（对比基础预测15分钟）
- [ ] AC-02: 个人修正系数学习输出个人类型标签（耐力型/速度型/均衡型）及对应修正系数
- [ ] AC-03: Riegel曲线拟合基于个人历史比赛数据，输出个人化指数（标准值1.06，个人偏差范围0.95-1.15）
- [ ] AC-04: 赛前状态修正集成v0.19 BodySignalEngine，疲劳度高时预测成绩自动下调，恢复状态好时上调
- [ ] AC-05: 预测历史验证记录每次预测vs实际结果，输出MAE和偏差方向统计
- [ ] AC-06: 数据不足时(比赛记录<3次)，降级为基础Jack Daniels公式预测，输出提示"基于3次历史比赛数据个人化校准"
- [ ] AC-07: 新增CLI命令 `predict race --distance marathon`，输出标注"🎯 基于N次历史比赛数据个人化校准"
- [ ] AC-08: 新增Agent工具 `predict_race_result`，返回RacePredictionResult数据结构
- [ ] AC-09: 全马预测输出包含配速策略建议，含每 5km 分段配速
...
class PredictionRecord:
    ...
    deviation_seconds: float | None
    deviation_pct: float | None
```

**前置依赖**:
- RacePredictionEngine.predict() 可复用核心预测逻辑（降级方案）
- RacePredictionEngine.vdot_to_time() 可复用VDOT→时间转换
- GoalPredictionEngine.evaluate_goal() 可复用目标评估逻辑
- BodySignalEngine(v0.19) 提供赛前身体状态数据

---

##### REQ-0.20-03：ML伤病风险预测

**需求描述**: 综合训练负荷时序特征和身体信号数据，使用ML分类模型预测伤病风险，替代多因子阈值判断，提供3周前置预警和可解释风险因子
...
| 规则基线 | ACWR/单调性/连续高强度/静息心率异常规则 | 冷启动兜底 | 无需训练数据 |

**验收标准**:

- [ ] AC-01: 数据充足时(18个月+/心率完整度>80%)，使用ML模型预测伤病风险，3周前置预警召回率>75%（对比基础预测1周）
- [ ] AC-02: 时序特征提取≥4类特征：负荷变化率特征、TSB趋势特征、静息心率偏移特征、HRV变化特征
- [ ] AC-03: 多模态融合整合TrainingLoadAnalyzer(ATL/CTL/TSB) + BodySignalEngine(疲劳度/恢复状态) + HRVAnalyzer(RMSSD/静息心率)
- [ ] AC-04: 风险时间线预测输出未来7/14/21天伤病概率曲线，概率>60%时触发预警
- [ ] AC-05: 可解释风险因子输出Top3触发因子，贡献度=该因子SHAP绝对值均值/所有因子SHAP绝对值均值之和，如"ATL/CTL比率持续>1.3（贡献35%）"
- [ ] AC-06: 数据不足时，降级为基础多因子评估（v0.19 InjuryRiskAnalyzer），输出提示
- [ ] AC-06b: 数据中等时(100-300条)，启用逻辑回归+CalibratedClassifierCV参数化基线，输出提示"📊 参数化模型预测"
- [ ] AC-07: 新增CLI命令 `predict injury-risk --days 21`，输出未来3周风险概率曲线
...
**前置依赖**:
- InjuryRiskAnalyzer.calculate_injury_risk() 可复用核心评估逻辑（降级方案）
- TrainingLoadAnalyzer 可复用ATL/CTL/TSB计算
- BodySignalEngine(v0.19) 可复用身体信号预警
- HRVAnalyzer(v0.19) 可复用静息心率趋势和HRV数据
...
- [ ] AC-03: 伤病记录持久化存储至 `~/.nanobot-runner/injury_labels/`，可用于ML模型训练标签
...
**需求描述**: 管理个人ML预测模型的生命周期，支持模型训练、版本管理、准确性追踪和增量学习，确保模型越用越准
...
- [ ] AC-01: 个人模型训练支持VDOT/比赛/伤病三种模型，训练完成输出模型指标（训练样本数、特征数、验证误差）
- [ ] AC-02: 模型版本管理保存至本地目录(`~/.nanobot-runner/models/`)，支持按版本号回滚，模型文件包含版本元数据
- [ ] AC-03: 预测准确性追踪记录每次预测结果，当实际结果可用时自动计算偏差，输出MAE和偏差趋势
- [ ] AC-04: 增量学习在新数据积累≥50条时自动触发模型更新，更新过程异步执行不阻塞CLI
- [ ] AC-05: 重训练触发条件：首次数据≥100条/新增50条/距上次>30天/用户手动/训练失败回退基线
- [ ] AC-06: 单模型训练时间<5分钟
- [ ] AC-07: 新增CLI命令 `predict model status` 查看模型状态，`predict model train --type vdot` 手动触发训练
...
**需求描述**: 评估当前数据是否满足ML预测要求，提供数据质量报告、积累建议和解锁进度提示
...
- [ ] AC-01: 数据质量报告输出6个维度评估：时间跨度、记录数量、训练频率、比赛记录、心率完整度、身体信号可用性
- [ ] AC-02: 每个维度输出当前值、目标值、达标状态(✅/🔒)，整体输出ML预测解锁状态
- [ ] AC-03: 数据积累建议针对未达标维度给出具体行动建议，如"还需积累6个月数据，建议保持每周3次以上记录"
- [ ] AC-04: 解锁进度提示输出百分比进度条，如"VDOT趋势ML预测: 🔒 67% (12/18个月)"
- [ ] AC-05: 新增CLI命令 `predict status`，输出完整的数据充足度报告
...
**需求描述**: 基于充足历史数据提供深度分析能力，帮助跑者理解训练响应性、发现最佳训练窗口

**功能要点**:

| 子功能 | 说明 | 数据需求 |
|--------|------|----------|
| 训练响应性分析 | 分析用户对不同训练刺激的反应 | 24个月+，多种训练模式 |
| 最佳训练窗口预测 | 预测何时是突破VDOT的最佳时机 | 完整周期数据 |
| 年度周期规划建议 | 基于历史数据建议年度训练周期 | 24个月+ |

**验收标准**:

- [ ] AC-01: 训练响应性分析输出不同训练类型(间歇/阈值/长距离)对VDOT的提升效果排名
- [ ] AC-02: 最佳训练窗口预测基于CTL-VDOT关联分析，输出"未来2-4周是突破VDOT的最佳窗口"
- [ ] AC-03: 年度周期规划建议基于历史数据识别个人年度周期模式，输出建议的训练高峰/恢复周期

---

#### 3.1.6 v0.20.0 用户交互设计
...
| `manage_prediction_model` | 模型管理(P1) | action, model_type | ModelManagementResult |
| `report_injury` | 伤病报告提交 | injury_type, severity, date | InjuryReportResult |
| `predict_training_response` | 训练响应预测 | session_type, duration_min, intensity | TrainingResponse |

**AI交互场景**:

| 场景 | 用户输入 | AI响应 |
|------|----------|--------|
| 数据充足咨询 | "我的数据够开启ML预测了吗？" | 展示数据充足度报告，明确解锁进度 |

---

#### 3.1.7 v0.20.0 新增存储设计
...
| REQ-0.21-01 | 跑者状态向量（5维度≥15指标/自动聚合） | P0 | REQ-0.20-01, REQ-0.20-02, REQ-0.20-03 |
| REQ-0.21-02 | What-If推演引擎（模拟/对比/综合评分） | P0 | REQ-0.21-01 |
| REQ-0.21-03 | 最优计划搜索（约束条件/最优搜索） | P1 | REQ-0.21-02 |
