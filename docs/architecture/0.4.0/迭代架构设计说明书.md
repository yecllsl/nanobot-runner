...
graph TD
    ...
    subgraph 数据层
        ...
        Storage --> L[(Profile JSON<br/>🆕 画像数据)]
...
| 数据类型 | 存储路径 | 格式 |
|---------|---------|------|
| 活动数据 | `~/.nanobot-runner/data/activities_{year}.parquet` | Parquet |
| 用户画像 | `~/.nanobot-runner/data/profile.json` | JSON |
| 训练计划 | `~/.nanobot-runner/data/plans/{plan_id}.json` | JSON |
| 配置文件 | `~/.nanobot-runner/config.json` | JSON |
| 日志文件 | `~/.nanobot-runner/logs/` | Log |
