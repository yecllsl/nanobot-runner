...
> **需求来源**: REQ_需求规格说明书.md (v20.0)
> **对齐依据**: 产品规划方案.md (v18.0)
...
| 交互升级 | v0.26-v0.32 | 底座升级+新特性(WebUI基础/数据可视化/管理控制台/Gateway增强/系统健壮性/底座升级) | ✅ 完成 |
...
| **WebUI基础** (v0.27) | RunnerProviderAdapter扩展(WebSocket配置+webui_enabled) + ConfigManager.get_webui_config() + Gateway `--webui` 标志 | ADR-015(config节+ProviderAdapter翻译) / ADR-016(CLI+config双重控制) / ADR-017(默认token+token_issue_path短期令牌) |
...
| `src/core/config/manager.py` | 修改 | 新增 `get_webui_config() -> dict[str, Any]` 方法（含环境变量覆盖） |
...
**核心交付**：新增 3 个路由模块（plan.py/evolution.py/settings.py），13 个 API 端点；新增 4 个前端页面 + 3 个 API 客户端；训练计划日历/列表双视图 + 执行进度环形图 + AI/手工双模式调整；进化引擎状态面板 + 提示参数滑块调优 + 月度进化报告；设置中心个人资料管理 + 系统配置查看。详细设计见Git历史版本与架构评审报告-v0.29.0.md。

关键架构决策：ADR-023(计划调整AI模式跳转8765)、ADR-024(自建CSS Grid日历组件)、ADR-025(手动刷新进化状态)。

***

## 12.5 v0.30.0-v0.31.0 系统健壮性提升 ✅ 已完成

v0.30.0 飞书 Gateway 增强：飞书Gateway增强、盈亏分析+结算、代码质量修复。v0.31.0 系统健壮性提升：代码质量优化、系统健壮性提升。v0.30.0-v0.31.0 无新增架构模块，专注代码质量修复与系统健壮性提升。详细设计见Git历史版本。
...
| Patch | 替代方案 |
|-------|---------|
| `_patch_websocket_settings_api()` | 配置物理分离（nanobot_config.json） |
| `_patch_load_config()` | 配置物理分离（nanobot_config.json） |
| `_patch_save_config()` | 配置物理分离（nanobot_config.json） |
...
| 组件 | 文件 | 职责 |
|------|------|------|
| ConfigInjector（未实现） | — | **ADR-026 方案 B**：配置注入职责由 RunnerProviderAdapter 承担，nanobot_config.json 由 ConfigManager 直接管理（YAGNI，不补建独立模块） |
...
```
RunFlowAgent 配置 → ConfigManager(nanobot_config.json) + RunnerProviderAdapter → nanobot Config → Nanobot.from_config()
                                                              ↓
                                    ┌─────────────────────────┴──────────────────────────┐
                                    ↓                       ↓                            ↓
...
**实现修正（2026-07-18，方案 B）**：ConfigInjector 作为独立模块未实现（YAGNI）。v0.32.0 采用配置物理分离架构，`nanobot_config.json` 由 ConfigManager 直接管理，配置注入职责由 `RunnerProviderAdapter._from_nanobot_config()` 承担。无需独立的配置转换层，避免了过度设计。
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v26.0.0 | 2026-07-15 | **v0.32.0 发布修订 + 精简**：①§12 v0.29.0 从"设计中"更新为"已完成"并精简；②新增§12.5 v0.30.0-v0.31.0 系统健壮性提升摘要；③架构演进路线扩展至 v0.32；④对齐文档更新为 REQ v20.0 + 产品规划 v18.0 |
