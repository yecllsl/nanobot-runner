...
##### 18.3.2.1 工具管理器（ToolManager）

**职责**: 统一管理外部工具的注册、发现、配置和调用
...
##### 18.3.2.2 天气服务接入（WeatherService）
...
##### 18.3.2.4 健康数据同步（HealthDataSync）
...
```python
# src/core/tools/mcp_integration.py
...
class MCPIntegration:
    ...
    def register_tool_to_mcp(self, tool: Tool) -> bool:
        """将工具注册到MCP系统
    ...
    def load_claude_desktop_config(self) -> dict[str, Any]:
        """加载Claude Desktop配置
        
        Returns:
            dict: Claude Desktop配置
        """
        pass
```
