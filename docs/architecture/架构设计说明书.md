...
> **更新日期**: 2026-07-15
> **当前基线**: v0.32.0
> **版本目标**: v0.32.0 nanobot-ai 0.2.2 升级与配置分离 ✅ 已完成
...
## 13. v0.32.0 nanobot-ai 0.2.2 升级 ✅ 已完成

### 13.1 升级目标

将底座 nanobot-ai 从 0.2.1 升级至 0.2.2，移除 4 项 monkey-patch 技术债，集成 9 项新功能。

### 13.2 核心变更

#### 13.2.1 移除的 monkey-patch（4 项）

| Patch | 替代方案 |
|-------|---------|
| `_patch_websocket_settings_api()` | ConfigInjector 配置注入 |
| `_patch_load_config()` | ConfigInjector 配置注入 |
| `_patch_save_config()` | ConfigInjector 配置注入 |
| `_patch_agent_loop_private_api()` | AgentLoopAdapter 封装 |

#### 13.2.2 新增组件（5 个）

| 组件 | 文件 | 职责 |
|------|------|------|
| ConfigInjector | `src/core/config_injector.py` | 通过 SDK 配置注入替代 monkey-patch |
| SDKAdapter | `src/core/sdk_adapter.py` | 编程式 Agent 调用入口 |
| RuntimeEventHook | `src/core/transparency/runtime_event_hook.py` | 订阅运行时事件总线 |
| DynamicProviderRegistry | `src/core/provider_adapter.py` | 自定义 Provider 动态注册 |
| CronSessionDelivery | `src/core/plan/cron_callback.py` | Cron 会话绑定（接入 nanobot 0.2.2 原生） |

#### 13.2.3 新增功能（9 项）

1. SDK 编程式 Agent 调用（Nanobot.from_config/stream/run）
2. 运行时事件总线（turn_completed/session_turn_started/run_status_changed）
3. WebUI SSE 端点（/api/runtime-events）
4. 自定义 Provider WebUI 配置入口
5. 38 个内置 LLM Provider 集成（mistral/groq/nvidia 等）
6. 语音转录配置（TranscriptionConfig）
7. Cron 会话绑定（origin_delivery_context）
8. Hook 生命周期增强（before_run/after_run）
9. Dream 两阶段记忆适配

### 13.3 集成架构

```
RunFlowAgent 配置 → ConfigInjector → nanobot Config → Nanobot.from_config()
                                                              ↓
                                    ┌─────────────────────────┴──────────────────────────┐
                                    ↓                       ↓                            ↓
                            RuntimeEventHook        DynamicProviderRegistry         SDKAdapter
                                    ↓                       ↓                            ↓
                            WebUI SSE 端点          WebUI 设置中心              编程式调用入口
```

### 13.4 ADR 决策记录

#### ADR-026：配置注入替代 monkey-patch

**决定**：使用 ConfigInjector 将 RunFlowAgent 配置转换为 nanobot Config 对象，替代运行时 patch。

**理由**：monkey-patch 依赖 nanobot 私有 API，升级脆弱且难以维护。配置注入使用 nanobot 公开 API（Config Pydantic 模型），向前兼容性更好。

#### ADR-027：SDK 编程式调用入口

**决定**：新增 SDKAdapter 封装 Nanobot SDK（from_config/stream/run），作为编程式 Agent 调用的统一入口。

**理由**：nanobot 0.2.2 提供了正式的 SDK API，封装后可隔离 SDK 变更影响，同时提供流式和非流式两种调用方式。

***

| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v25.0.0 | 2026-07-13 | **v0.32.0 nanobot-ai 0.2.2 升级**：①新增§13 v0.32.0 升级架构设计；②移除4项monkey-patch，新增ConfigInjector配置注入；③新增SDKAdapter编程式Agent调用入口；④新增RuntimeEventHook运行时事件订阅；⑤新增DynamicProviderRegistry自定义Provider注册；⑥新增Cron会话绑定；⑦集成语音转录配置；⑧新增ADR-026配置注入决策、ADR-027 SDK调用入口决策 |
