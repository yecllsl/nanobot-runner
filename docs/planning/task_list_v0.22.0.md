...
| T-01-02 | 执行UAT-v0.5~v0.17模块验证 | 按UAT测试指南执行v0.5+(数据导入/查询/分析)、v0.10+(计划/报告)、v0.17(MCP/Gateway/Cron/透明化/偏好/技能)模块的UAT测试，含Gateway异常场景与Agent Chat增强测试（UAT-101~UAT-115） | T-01-01 | 16 | 按版本模块逐项执行，记录测试结果，输出测试记录表（通过/失败/阻塞）；Gateway/Agent异常用例(P0)100%执行 |
...
| T-01-04 | 执行UAT-性能测试 | 验证关键操作响应时间：ML预测<5秒、推演<10秒、状态聚合<3秒、批量导入性能；Gateway并发处理<15秒/条、大数据量查询<10秒 | T-01-02 | 6 | 性能测试报告，所有指标符合v0.21基线；Gateway并发/超时指标达标 |
...
| 并行组E | Gateway异常UAT(UAT-101~106) ∥ Agent Chat增强UAT(UAT-107~112) | T-01-01完成，Gateway服务可用 |
...
| Gateway并发/超时测试需真实飞书环境 | 中 | UAT-103/104无法执行 | 准备测试飞书应用或使用Mock替代 |
| Gateway长时间运行稳定性测试(UAT-114) | 低 | 24小时测试周期长 | 降级为4小时加速验证，记录内存趋势 |
| 消息队列积压测试(UAT-115)需模拟阻塞 | 中 | 无法模拟真实积压场景 | 编写测试脚本自动化发送100条消息 |
...
| 维度 | 标准 | 测量方式 |
|------|------|----------|
| UAT通过率 | P0用例100%，P1用例≥90%（含UAT-101~115 Gateway/Agent增强） | UAT测试报告 |
