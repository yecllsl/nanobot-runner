...
| 里程碑 | 时间节点 | 交付物 | 任务范围 |
|--------|---------|--------|---------|
| M1: 画像系统实现 | Day 3 | 用户画像引擎 + 双存储持久化 + 保鲜期机制 | T001-T003 |
| M2: 训练计划实现 | Day 7 | 训练计划引擎 + 日历同步（含Webhook集成） | T004-T006 |
| M3: 飞书集成完成 | Day 9 | 机器人 Channel + Agent 提示词配置 + 记忆更新工具 | T007-T009 |
...
**任务描述**:
实现画像数据的双存储持久化（MEMORY.md + profile.json）。

**详细子任务**:
1. 实现 `save_profile_json()` 方法（profile.json）
2. 实现 `load_profile_json()` 方法
3. 实现 `save_memory_md()` 方法（MEMORY.md）
4. 实现 `load_memory_md()` 方法
5. 实现 `sync_dual_storage()` 双存储同步方法
6. 实现 `merge_profile_to_md()` 智能合并方法
7. 定义存储路径 (`~/.nanobot-runner/data/profile.json`, `~/.nanobot-runner/memory/MEMORY.md`)

**验收标准**:
- [ ] profile.json 可正确保存和加载
- [ ] MEMORY.md 可正确保存和加载
- [ ] 双存储同步逻辑正确
- [ ] 智能合并保留 Agent 观察笔记
- [ ] 异常情况有完整处理
...
#### T006: 飞书反向同步与Webhook集成

| 属性 | 内容 |
|------|------|
| **任务ID** | T006 |
| **任务名称** | 飞书反向同步与Webhook集成 |
| **优先级** | P0 |
| **预估工时** | 10h |
| **前置依赖** | T005 |
| **负责模块** | notify/feishu_calendar.py |
...
6. **新增**: 实现 Webhook 路由配置
7. **新增**: 实现飞书事件订阅配置
8. **新增**: 实现 Webhook 签名验证
...
**详细子任务**:
1. 编写 `test_memory_init.py` 记忆系统初始化测试
2. 编写 `test_profile.py` 画像引擎测试
3. 编写 `test_training_plan.py` 训练计划测试
4. 编写 `test_feishu_calendar.py` 日历同步测试
...
**输出产物**:
- `tests/unit/test_memory_init.py`
...
**任务描述**:
实现新增工具类并注册到 ToolRegistry，包含记忆更新工具。
...
9. **新增**: 实现 `UpdateMemoryTool`（Agent专用记忆更新工具）
10. 实现 `create_tools_v2()` 工厂函数

**验收标准**:
- [ ] 所有工具类实现完整
- [ ] 工具 schema 格式正确
- [ ] 工具可正确注册到 ToolRegistry
- [ ] UpdateMemoryTool 可正确更新 MEMORY.md
...
| T008 | T007 | T009 |
| T009 | T008, T002-1 | T017 |
| T010 | T001 | T017 |
| T011 | T001 | T012 |
| T012 | T011 | T017 |
| T013 | T001-T006, T002-1 | T014 |
| T014 | T001-T006, T002-1, T013 | T017 |
| T015 | T007, T008 | T017 |
| T016 | T001 | T017 |
| T017 | T001-T016 | T019 |
| T018 | T006, T017 | - |
...
```
T002-1 → T002 → T003 → T013 → T014 → T017 → T019 → T020
(最长路径，约 12 个工作日)
```
...
| 需求编号 | 需求名称 | 验收标准 | 对应任务 |
|---------|---------|---------|---------|
| FR-001 | 用户画像系统 | 画像数据准确、双存储持久化、保鲜期机制、异常过滤 | T001-T003, T002-1 |
| FR-002 | 训练计划生成 | 计划科学、动态调整、阶段配置 | T004 |
| FR-003 | 飞书日历同步 | 双向同步、Webhook集成、冲突处理 | T005-T006 |
| FR-004 | 飞书机器人交互 | 协议转换正确、Agent统一处理、记忆更新工具 | T007-T009 |
...
| MEMORY.md 格式被破坏 | 低 | 中 | 使用 HTML 注释块保护统计数据 |
...
| v1.1 | 2026-03-19 | 架构评审修订：新增T002-1记忆系统初始化、修订T002双存储持久化、修订T006 Webhook集成、修订T009增加UpdateMemoryTool | 架构师智能体 |

---

**文档状态**: 已发布  
**发布版本**: v0.4.0-plan-v1.1
